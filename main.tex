...
\begin{frame}
	...
	By ``Maps into covering spaces'' it is also surjective.
	    ...
	    \pause
	\item If $q\colon Z\to X$ is an intermediate connected cover, then the map $Y\to Z$ is a cover as well (local on $Z$, hence on $X$, hence we may assume that this map has the form $X\times F_{Y}\to X\times F_{Z}$).
	    \pause
...
\begin{frame}
	...
	\item Define $\varphi(q([x])):=s$.
	    ...
	    \pause
	\item If $g'\cdot q([x])=g\cdot q([x])$, then $g'g^{-1}\in G_{q([x])}=\Aut(\tilde{X}_{x}\mid Y)$, which is $G_{s}$ by construction.
	    Hence we can define a $G$-equivariant map $\varphi\colon p^{-1}(x) \to S$ by $y=g\cdot q([x]) \mapsto g\cdot s$.
	    \pause
	\item If $g\cdot s=g'\cdot s$, then again $g'g^{-1}\in G_{s}=G_{y}$, so $\varphi(y)=$
    \end{enumerate}
\end{frame}
